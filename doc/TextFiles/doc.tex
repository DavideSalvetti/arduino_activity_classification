...
\end{figure}
Come blocco di apprendimento è stato scelto un classificatore. La rete neurale riceve in ingresso 400 features ricavate dal blocco DSP precedente e le analizza tramite due Dense Layer, composti rispettivamente da 32 e 16 neuroni, ed un Output Layer che fornisce in uscita la probabilità di appartenenza delle features in ingresso alle 4 classi. La predizione finale è la classe con probabilità maggiore.\todo{matteo: io non so bene cosa è quella matrice e nemmeno questa parte. lascio a te giustificarla e inserirla eventualmente. davide: siccome non so bene nemmeno io non la metto. Meglio non tirarsi la zappa sui piedi.}
Una volta sviluppato il modello è possibile testarlo sui dati di test (\Fig\ref{fig:model_test}). \todo{matteo: siccome non so bene il tutto non posso risponderti se è o non è over. per le mie conoscenze più siamo generici meglio è perché almeno io non saprei giustificare le scelte. davide: meglio.}
\begin{figure}[h!]
	\centering
